...
int uplowPartition(int *x, int, int, int);
int N;
...
int main(int argc, char *argv[]){
	...
	shmem_barrier_all();
	check = uplowPartition(A,next_pivot, npes, me);
	shmem_barrier_all();	
	printf("(%d)",me);	
...
int uplowPartition(int *A, int next_pivot, int npes, int me){
	int temp,j,i= -1;
//	int *temp_arr;
	for(j=0;j<N/npes;j++){
		if(A[j] <next_pivot) {
